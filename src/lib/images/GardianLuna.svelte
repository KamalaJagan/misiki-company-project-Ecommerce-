<script>
import afiv9 from '$lib/assets/dis/G1.webp'
import afiv1 from '$lib/assets/gardian/88.avif'
import afiv2 from '$lib/assets/gardian/af.avif'
import afiv3 from '$lib/assets/gardian/aff.avif'
import afiv4 from '$lib/assets/gardian/afff.avif'
import afiv5 from '$lib/assets/gardian/A20.webp'
import afiv6 from '$lib/assets/gardian/A21.webp'
import afiv7 from '$lib/assets/gardian/A22.webp'
import afiv8 from '$lib/assets/gardian/A23.webp'
let gardians = [
	{
		image: afiv1
	},
	{
		image: afiv2
	},
	{
		image: afiv3
	},
	{
		image: afiv4
	}
]
let gards = [
	{
		image: afiv5,
		name: 'Jameela Open Round Hoops',
		amount: '$26.95'
	},
	{
		image: afiv6,
		name: 'Bold Cuffs',
		amount: '$14.95'
	},
	{
		image: afiv7,
		name: 'Angel Wing Satellite Chain Necklace',
		amount: '$25.95'
	},
	{
		image: afiv8,
		name: 'Small Circle Studs',
		amount: '$12.95'
	}
]
// let boxs = [
// 	{

// 		text:"Eco friendly",
// 		text1:"Ava Martin",
// 		paragraph:"on",
// 		text2:"Mar 16, 2022",
// 		text3:"Love the eco friendly packaging",
// 		text4:"Report as Inappropriate",
// 	},
// 	{
// 		text:"Amazing service",
// 		text1:"Charlotte Moore",
// 		paragraph:"on",
// 		text2:"Mar 11, 2022",
// 		text3:"Recommended by a friend. Was not disappointed. Fast delivery and amazing customer service.",
// 		text4:"Report as Inappropriate",
// 	},
// 	{
// 		text:"Awesome product",
// 		text1:"Liam Perez",
// 		paragraph:"on",
// 		text2:"Feb 27, 2022",
// 		text3:"So impressed I placed another order as soon as I received my first delivery",
// 		text4:"Report as Inappropriate",
// 	},
// 	{
// 		text:"Fast delivery",
// 		text1:"Sophia Jackson ",
// 		paragraph:"on",
// 		text2:"Jan 11, 2022",
// 		text3:"Arrived well before I expected it and in super cute packaging.",
// 		text4:"Report as Inappropriate",
// 	},
// 	{
// 		text:"Perfect",
// 		text1:"Noah Lee",
// 		paragraph:"on",
// 		text2:"Jan 06, 2022",
// 		text3:"Gift for my partner - they loved it!",
// 		text4:"Report as Inappropriate",
// 	},
// ];
</script>

<!-- --------------------------------------------Image details and choices start ----------------------------------------------------- -->
<div class="flex gap-2 ml-4 text-xs mt-5 lg:ml-[30vh] lg:mt-10 m-2">
	<button class="hover:underline">Home</button>
	<h3>Guardian Luna Blue Topaz Necklace</h3>
</div>
<div class="grid grid-flow-row grid-cols-1 lg:grid-cols-2 mt-6 font-san-serif text-sm">
	<div>
		<img
			src="{afiv9}"
			class=" p-4 lg:ml-[28vh] h-[70vh] lg:h-[130vh] lg:w-[89vh] w-full object-cover"
			alt="" />
		<div class="hidden md:flex flex-row gap-5 mt-6 lg:ml-[28vh] justify-center items-center">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
				viewBox="0 0 24 24"
				stroke-width="1.5"
				stroke="currentColor"
				class="w-6 h-6">
				<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"></path>
			</svg>

			<img src="{afiv9}" class="h-32 w-24 object-cover" alt="." />
			<img
				src="https://cdn.shopify.com/s/files/1/0666/9309/products/Guardian_Luna_Blue_Topaz_Necklace_1_1100x.jpg?v=1580811048"
				class="h-32 w-24 object-cover"
				alt="." />
			<img
				src="https://cdn.shopify.com/s/files/1/0666/9309/products/Guardian_Luna_Blue_Topaz_Necklace_2_1100x.jpg?v=1580811048"
				class="h-32 w-24 object-cover"
				alt="." />
			<img
				src="https://cdn.shopify.com/s/files/1/0666/9309/products/IMG_1711_2-cropped_1100x.jpg?v=1648501269"
				class="h-32 w-24 object-cover"
				alt="." />
			<img
				src="https://cdn.shopify.com/s/files/1/0666/9309/products/IMG_1774-cropped_1100x.jpg?v=1648501269"
				class="h-32 w-24 object-cover"
				alt="." />
			<!-- <img src={afiv9} class="h-5 w-5" alt="." /> -->
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
				viewBox="0 0 24 24"
				stroke-width="1.5"
				stroke="currentColor"
				class="w-6 h-6">
				<path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"></path>
			</svg>
		</div>
	</div>
	<div class=" lg:ml-[14vh] p-4">
		<h1 class="text-2xl font-serif">Guardian Luna Blue Topaz Necklace</h1>
		<div class="flex flex-row mt-4 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<div class="hover:underline ml-2">(6 total reviews)</div>
		</div>
		<div class="text-xl">$69.95</div>
		<div class="flex gap-1 mt-4">
			<a href="/shipping" class="underline hover:no-underline cursor-pointer">Shipping</a>
			<p>calculated at checkout.</p>
		</div>
		<div class="mt-3">Quantity</div>
		<button
			class="border-2 border-gray-400 h-14 mt-4 w-[27vh]  grid grid-cols-3 justify-center py-3 ">
			<div>-</div>
			<div>1</div>
			<div>+</div>
		</button>
		<button
			class="border-2 border-gray-400 h-12 lg:w-[61vh] w-full mt-8 tracking-widest hover:border-black"
			>ADD TO CART</button>
		<button
			class=" bg-gray-500 border-gray-400 h-12 lg:w-[61vh] w-full tracking-widest mt-8 hover:bg-yellow-500"
			>BAY IT NOW</button>
		<div class="font-bold text-base font-sans-serif mt-3">Guardian Luna Blue Topaz Necklace</div>
		<p class="gap-1 mt-4 text-sm">
			This is a demo store. This product can be bought on <a
				href="/"
				class="hover:underline italic cursor-pointer">Clover & Swift</a
			>.
		</p>
		<div class="flex ml-4 mt-4">
			<div>Material type</div>
			<div class="ml-6">18k Gold Vermeil</div>
		</div>
		<hr class=" lg:w-[61vh] mt-4" />
		<div class="flex ml-4 mt-2">
			<div>Stone</div>
			<div class="ml-[10vh]">Blue Topaz inlaid with 22k gold flakes</div>
		</div>
		<hr class=" lg:w-[61vh] mt-4" />
		<div class="flex ml-4 mt-2">
			<div>Chain length</div>
			<div class="ml-7">18-20 inch</div>
		</div>
		<hr class=" lg:w-[61vh] mt-4" />
		<div class="flex ml-4 mt-2">
			<div class="">
				Pendant
				<p>length / width</p>
			</div>
			<div class="ml-5">2cm</div>
		</div>
		<hr class=" lg:w-[61vh] mt-4" />
		<div class="flex ml-4 mt-2">
			<div>Packaging</div>
			<div class="ml-12 lg:mr-52">
				Comes beautifully displayed on card. Clover and Swift cotton bag to keep your treasured
				pieces safe. All of our packaging is plastic free
			</div>
		</div>
		<hr class=" lg:w-[61vh] mt-4" />
		<div class="flex gap-1 mt-3">
			<svg
				aria-hidden="true"
				focusable="false"
				role="presentation"
				class="icon icon-in-stock w-3 mt-1"
				viewBox="0 0 12 10"
				><path
					fill-rule="evenodd"
					clip-rule="evenodd"
					d="M3.293 9.707l-3-3a.999.999 0 1 1 1.414-1.414l2.236 2.236 6.298-7.18a.999.999 0 1 1 1.518 1.3l-7 8a1 1 0 0 1-.72.35 1.017 1.017 0 0 1-.746-.292z"
					fill="#228B22"></path
				></svg>
			<div class="flex gap-1">
				Pickup available at <p class="font-bold">Fore St Store</p>
			</div>
		</div>
		<div class="text-xs ml-4 mt-1">Usually ready in 24 hours</div>
		<div class="mt-3 underline hover:no-underline hover:text-gray-400 text-xs ml-4 cursor-pointer">
			Check availability at other stores
		</div>
		<div class="mt-4 h-[54vh] lg:w-[61vh] w-full bg-gray-50">
			<div class="text-lg ml-4 py-4">Don't forget these...</div>
			<div class="grid grid-flow-row lg:grid-cols-4 grid-cols-2">
				<img
					src="https://cdn.shopify.com/s/files/1/0666/9309/products/2F7932E9-A628-4538-945E-C4653F2F79BB_550x.jpg?v=1580827001"
					alt="/"
					class="h-30 w-20 object-cover ml-4" />
				<div class="">
					<div>Blue Topaz Wishing Locket</div>
					<div class="mt-2">$49.95</div>
					<div class="hidden">
						<button
							class="flex flex-flow-row border-2 mt-3 w-32 py-1 px-2 justify-between gap-1 items-center"
							>Text choice<svg
								fill="none"
								viewBox="0 0 24 24"
								stroke-Width="{1.5}"
								stroke="currentColor"
								stroke-linejoin="round"
								stroke-linecap="round"
								class="w-4 h-7  hover:text-gray-500">
								<path d="M19.5 8.25l-7.5 7.5-7.5-7.5"></path>
							</svg></button>
					</div>
				</div>
				<button
					class="border-2 h-16 w-44w8
				 lg:w-36 hover:bg-yellow-200 hover:text-white text-lg ml-32 lg:ml-16">MAKE A SELECTION</button>
			</div>
			<div class="grid grid-flow-row lg:grid-cols-4 grid-cols-2 mt-8">
				<img
					src="https://cdn.shopify.com/s/files/1/0666/9309/products/Clover_Swift_Leah-95websize_550x.jpg?v=1580810690"
					alt="/"
					class="h-30 w-20 object-cover ml-4" />
				<div class="">
					<div>Guardian Blue Topaz Shield Satellite Chain Necklace</div>
					<div>$34.95</div>
				</div>
				<button
					class="border-2 h-16 w-44 lg:w-36 bg-gray-800 hover:bg-yellow-200 text-white justify-center text-lg ml-32 lg:ml-16"
					>ADD TO CART</button>
			</div>
		</div>
	</div>
</div>

<!-- -------------------------------------------Image details and choices ends----------------------------------------------------------->

<!------------------------------------------------------- custmor reviews will be started ------------------------------------------- -->

<div class="border-2 border-gray-300 lg:ml-[28vh] lg:mr-[28vh] mb-4 mt-16 m-4 font-sans-serif">
	<div class="p-7">
		<h1 class="text-3xl font-serif">Customer Reviews</h1>
		<div class="flex flex-row mt-3 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-6 w-6"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-6 w-6"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-6 w-6"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-6 w-6"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-6 w-6"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<div class="flex flex-row justify-between">
			<h3>Based on 6 reviews</h3>
			<h3 class="hover:underline cursor-pointer">Write a review</h3>
		</div>
	</div>
	<hr class="mr-7 ml-7" />
	<div class="p-7">
		<div class="flex flex-row text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<h3 class="mt-2">Eco friendly</h3>
		<h3 class="italic font-bold flex">
			Ava Martin <p class="font-normal px-1">on</p>
			Mar 16, 2022
		</h3>
		<h3 class="mt-3">Love the eco friendly packaging</h3>
		<h5
			class="hover:underline cursor-pointer flex flex-row-reverse md:items-left lg:justify-start mt-4 text-xs">
			Report as Inappropriate
		</h5>
	</div>
	<hr class="ml-7 mr-7" />
	<div class="p-7">
		<div class="flex flex-row mt-4 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<h3 class="mt-2">Amazing service</h3>
		<h3 class="italic mt-1 font-bold flex">
			Charlotte Moore <p class="font-normal px-1">on</p>
			Mar 11, 2022
		</h3>
		<h3 class="mt-3">
			Recommended by a friend. Was not disappointed. Fast delivery and amazing customer service.
		</h3>
		<h3
			class="hover:underline cursor-pointer flex flex-row-reverse md:items-left lg:justify-start mt-4 text-xs">
			Report as Inappropriate
		</h3>
	</div>
	<hr class="ml-7 mr-7" />
	<div class="p-7">
		<div class="flex flex-row mt-4 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<h3>Awesome product</h3>
		<h3 class="italic mt-1 font-bold flex">
			Liam Perez <p class="font-normal px-1">on</p>
			Feb 27, 2022
		</h3>
		<h3>So impressed I placed another order as soon as I received my first delivery</h3>
		<h3
			class="hover:underline cursor-pointer flex flex-row-reverse md:items-left lg:justify-start mt-4 text-xs">
			Report as Inappropriate
		</h3>
	</div>
	<hr class="ml-7 mr-7" />
	<div class="p-7">
		<div class="flex flex-row mt-4 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<h3>Fast delivery</h3>
		<h3 class="italic mt-1 font-bold flex">
			Sophia Jackson <p class="font-normal px-1">on</p>
			Jan 11, 2022
		</h3>
		<h3>Arrived well before I expected it and in super cute packaging.</h3>
		<h3
			class="hover:underline cursor-pointer flex flex-row-reverse md:items-left lg:justify-start mt-4 text-xs">
			Report as Inappropriate
		</h3>
	</div>
	<hr class="ml-7 mr-7" />
	<div class="p-7">
		<div class="flex flex-row mt-4 text-gray-800 ">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				class="h-5 w-5"
				viewBox="0 0 24 24"
				fill="currentColor">
				<path
					fill-Rule="evenodd"
					d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
					clip-Rule="evenodd"></path>
			</svg>
		</div>
		<h3>Perfect</h3>
		<h3 class="italic mt-1 font-bold flex">
			Noah Lee
			<p class="font-normal px-1">on</p>
			Jan 06, 2022
		</h3>
		<h3>Gift for my partner - they loved it!</h3>
		<h3
			class="hover:underline cursor-pointer flex flex-row-reverse md:items-left lg:justify-start mt-4 text-xs">
			Report as Inappropriate
		</h3>
	</div>
	<hr class="ml-7 mr-7" />
	<div class="flex justify-between ml-7 mt-2 mb-2 mr-3">
		<div class="flex gap-1 items-center">
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
				viewBox="0 0 24 24"
				stroke-width="1.5"
				stroke="currentColor"
				class="w-3 h-3 mt-1">
				<path
					stroke-linecap="round"
					stroke-linejoin="round"
					d="M18.75 19.5l-7.5-7.5 7.5-7.5m-6 15L5.25 12l7.5-7.5"></path>
			</svg>

			<a href="/">Preview</a>
		</div>
		<div class="ml-4 items-center ">
			<a href="/" class="hover:underline hover:font-bold">1</a>
			<a href="/" class="hover:underline hover:font-bold">2</a>
		</div>
		<div class="mr-4 flex gap-1 items-center">
			<a href="/">Next</a>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
				viewBox="0 0 24 24"
				stroke-width="1.5"
				stroke="currentColor"
				class="w-3 h-3 mt-1">
				<path
					stroke-linecap="round"
					stroke-linejoin="round"
					d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5"></path>
			</svg>
		</div>
	</div>
</div>

<!-- ----------------------------------------------------Custmor reviews will be ended----------------------------------------------- -->

<!-- -------------------------------------------------As seen page will be started--------------------------------------------------- -->

<div class=" bg-[#ded1bd] lg:h-[300px] lg:w-[210vh] h-auto w-full font-sans-serif">
	<div class="text-center text-4xl mt-10">
		<a href="/" class="font-serif">As seen in</a>
	</div>
	<div class="flex justify-center flex-wrap gap-4 mt-8">
		{#each gardians as gard, bx}
			<img
				src="{gard.image}"
				class="lg:h-32 w-auto h-28 object-contain object-top flex-shrink-0"
				alt="..." />
		{/each}
	</div>
</div>

<!-- ---------------------------------------------------------As seen page will be ended------------------------------------------------ -->

<!-- ---------------------------------------------------images will be started ------------------------------------------------------- -->

<div class="mt-5 h-auto w-full p-4 font-sans-serif ">
	<h1 class="text-center mt-4 text-4xl text-gray-800 font-serif font-light">You may also like</h1>
	<div
		class="grid h-auto mt-5 grid-flow-row grid-cols-2 md:grid-cols-4 gap-8 mx-auto max-w-md lg:max-w-6xl bg-cover">
		{#each gards as grand, bx}
			<div class="hover:text-gray-500">
				<div class="overflow-hidden lg:h-96 h-auto w-auto">
					<img
						src="{grand.image}"
						class="object-cover h-full w-full transition duration-500 hover:scale-110 cursor-pointer"
						alt="....." />
				</div>
				<div class="text-center mt-4 mb-6 hover:text-gray-500 cursor-pointer">
					<div>{grand.name}</div>
					<!-- <div class="line-through text-xs">{grand.amount1}</div> -->
					<div>{grand.amount}</div>
				</div>
			</div>
		{/each}
	</div>
</div>

<!-- ---------------------------------------------------------Images will be ended -------------------------------------------------- -->
